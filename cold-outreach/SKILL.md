...
| `crm/tasks.md` | Manual follow-up tasks created after outreach replies can be logged and tracked in CRM Tasks. |
| `crm/SKILL.md` | Use the CRM CLI to bulk query or export leads generated by Cold Outreach campaigns. |
...
| `workflows` | Cold Outreach trigger events (`form_submitted`, `email_replied`, `email_clicked`) can be used as workflow triggers. |
| `meeting-links` | Meeting Agent booking links can be used as CTA links in Cold Outreach campaign emails. |
| `marketing/whatsapp-campaign.md` | Alternative bulk messaging channel. Use Cold Outreach for multi-step email/LinkedIn; use `marketing/whatsapp-campaign.md` for one-shot WhatsApp blasts. |
| `settings` | Personal Mailboxes (`/settings#personal_mailboxes`) and LinkedIn Account (`/settings#linkedin_account`) must be configured before campaigns can send. |
